...
const filterSlice = createSlice({
  ...
  reducers: {
    ...
    low_to_high: (state, action) => {
      const state_ = current(state);
      let tempArr =
        state_.searchResults.length > 0
          ? [...state_.searchResults]
          : [...state_.country];
    ...
    clearSearch: (state, action) => {
      const state_ = current(state);
      let tempArr = [...state_.country];
      state.searchResults = tempArr.map((value) => {
        return value;
      });
      console.log(state.searchResults);
    },
...
export const { searchItems, low_to_high, high_to_low, rating, clearSearch } =
  filterSlice.actions;
