...
export default class Rect extends PureComponent {
  ...
  render() {
    ...
    const { type: Tag, props } = this.props.children
    let tag = React.createElement(
      `${Tag}`,
      {
        ...props,
        ref: active ? null : this.setElementRef,
        // onClick: handleClickOutside,
        // onDoubleClick: handleClose,
        // id: id,
        onMouseEnter: () =>
          !active && !hover && this.setState({ active: 0, hover: 1 }),
        onMouseLeave: () =>
          !active && hover && this.setState({ active: 0, hover: 0 }),
        // style:
        //   (active || hover) && Tag !== 'img'
        //     ? {}
        //     : Tag !== 'img'
        //     ? { ...style, position: 'absolute' }
        //     : { ...style, position: active || hover ? 'unset' : 'absolute' },
      },
      props.children
    )
